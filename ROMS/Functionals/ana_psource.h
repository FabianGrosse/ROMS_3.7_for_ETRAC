...
#endif
      USE mod_sources
#if defined UPWELLING && defined TBNT_OUT
      USE mod_biology
#endif
...
!
        ...
        IF (Master.and.DOMAIN(ng)%SouthWest_Test(tile)) THEN
          ...
          END DO
        END IF
#elif defined UPWELLING && defined TBNT_OUT
        IF (Master.and.DOMAIN(ng)%SouthWest_Test(tile)) THEN
          Nsrc(ng)=1
          SOURCES(ng)%Dsrc(Nsrc(ng))=1.0_r8
          SOURCES(ng)%Isrc(Nsrc(ng))=10
          SOURCES(ng)%Jsrc(Nsrc(ng))=1
        END IF
#else
...
#if defined RIVERPLUME1 || \
    (defined UPWELLING && defined TBNT_OUT)
...
#  if defined UPWELLING && defined TBNT_OUT
              SOURCES(ng)%Tsrc(is,k,iNO3_)=1.0E-5_r8
#  endif
            END DO
          END DO
        END IF
