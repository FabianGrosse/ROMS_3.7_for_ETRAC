...
!
!svn $Id: fennel.h 854 2017-07-18 23:28:45Z arango $
!***********************************************************************
...
!
      ...
      USE mod_diags
#endif
#if defined TBNT_OUT && defined BIO_SEDIMENT && \
...
# endif
    ...
    defined DENITRIFICATION && defined SOLVE3D
      ...
      real(r8), intent(inout) :: TBNTflx2(LBi:,LBj:,:)
      real(r8), intent(inout) :: TBNTflx3(LBi:,LBj:,:,:)
      real(r8), intent(inout) :: TBNTsink(LBi:,LBj:,:,:)
#endif
...
# endif
    ...
    defined DENITRIFICATION && defined SOLVE3D
      ...
      real(r8), intent(inout) :: TBNTflx2(LBi:UBi,LBj:UBj,nTBNTflx2)
      real(r8), intent(inout) :: TBNTflx3(LBi:UBi,LBj:UBj,UBk,nTBNTflx3)
      real(r8), intent(inout) :: TBNTsink(LBi:UBi,LBj:UBj,UBk,nTBNTsink)
#endif
...
!
!  Alternative formulation for Schmidt number coefficients
! (Sc will be slightly smaller up to about 35 C) using the formulation
! proposed by Keeling et al. (1998, Global Biogeochem. Cycles, 12, 141-163).
!
...
!
!  Alternative formulation for Schmidt number coefficients
!  using the formulation of Wanninkhof (2014, L&O Methods, 12,351-362)
...
#if defined TBNT_OUT && defined BIO_SEDIMENT && \
    defined DENITRIFICATION && defined SOLVE3D
      real(r8), dimension(IminS:ImaxS,N(ng)) :: vol
#endif
...
!
!-----------------------------------------------------------------------
! If appropriate, initialize time-integrated TBNT arrays.
!-----------------------------------------------------------------------
!
      IF (((iic(ng).gt.ntsTBNT(ng)).and.                                &
     &     (MOD(iic(ng),nTBNT(ng)).eq.1)).or.                           &
     &    ((iic(ng).ge.ntsTBNT(ng)).and.(nTBNT(ng).eq.1)).or.           &
     &    ((nrrec(ng).gt.0).and.(iic(ng).eq.ntstart(ng)))) THEN
        DO ivar=1,nTBNTflx2
          DO j=Jstr,Jend
            DO i=Istr,Iend
              TBNTflx2(i,j,ivar)=0.0_r8
            END DO
          END DO
        END DO
        DO ivar=1,nTBNTflx3
...
!
        DO i=Istr,Iend
#if defined NENA || defined MCH_BIO
!
!  Use quadratic regression between "ROMS PAR" (which assumes PARfrac = 0.43%)
!  and SeaWiFS PAR derived by Jay O''Reilly:
!  Adjusted ROMS PAR = -4.9039+1.2347*ROMS PAR+0.00411*ROMS PAR*ROMS PAR
!  Note: Jay''s approximation assumes units of E/m2/d but here we need 
!        Watts/m2
!  That means:
!     For PAR_einst_new = a+b*PAReinst+c*PAReinst^2
!         where PAReinst=g*PARwatts (g=0.398)
!     PARwatts_new = a/g+b*PARwatts+c*g*PARwatts^2
!     hence: a_watts = -4.9039/0.398 = -12.3214
!            b_watts = b = 1.2347
!            c_watts = c*g = 0.0016 
!
          fac1=PARfrac(ng)*srflx(i,j)*rho0*Cp
	  PARsur(i)=-12.3214_r8+1.2347_r8*fac1+0.0016_r8*fac1*fac1
#else
...
!  modify AttFac.
#ifdef SABGOM_RUOYING
                AttFac=AttFac +                                         &
     &                 MAX(-0.0238_r8*Bio(i,k,isalt)+0.8848_r8,0.0_r8)
#endif
#ifdef LATTE
...
#elif defined NENA
!  Include a salinity-dependent attenuation coefficient to mimic 
!  attenuation due to CDOM. The slope and intercept of the relationship 
!  were chosen based on MARMAP data and a CDOM attenuation of 0.01 m-1
!  at BATS (based on Dave Siegel''s work)
!
        	fac1=MAX(-0.0255_r8*Bio(i,k,isalt)+0.9451_r8, 0.0_r8)
                Att=(AttSW(ng)+AttChl(ng)*Bio(i,k,iChlo)+   &
     &                  fac1)*(z_w(i,j,k)-z_w(i,j,k-1))
#elif defined MCH_BIO
# ifdef GREEN_LIGHT_ATT
!  Include salinity-dependent attenuation coefficient to mimic
!  attenuation due to CDOM. This is the relationship from Green et al. (2008)
!  which uses data from Lohrenz et al. (1999).
!
                fac1= 0.0022_r8*Bio(i,k,isalt)*Bio(i,k,isalt)           &
     &               -0.158_r8*Bio(i,k,isalt) + 3.03_r8
# else
!  Include salinity-dependent attenuation coefficient to mimic
!  attenuation due to CDOM. Slope and intercept were chosen based on MARMAP
!  data and a CDOM attenuation of 0.01 m-1 at BATS. 
!  Note: This relationship is linear Type II model (not the quadratic
!        relationship Jay proposed.
!
                fac1=MAX(-0.0238_r8*Bio(i,k,isalt)+0.8848_r8, 0.0_r8)
# endif
                Att=(AttSW(ng)+AttChl(ng)*Bio(i,k,iChlo)+    &
     &              fac1)*(z_w(i,j,k)-z_w(i,j,k-1))
#elif defined BYECS2
...
!
                ...
                cff4=mu*Bio(i,k,iPhyt)*L_NO3/MAX(MinVal,LTOT)/MAX(MinVal,Bio(i,k,iNO3_))
                cff5=mu*Bio(i,k,iPhyt)*L_NH4/MAX(MinVal,LTOT)/MAX(MinVal,Bio(i,k,iNH4_))
                cff6=R_P2N(ng)*mu*Bio(i,k,iPhyt)/MAX(MinVal,Bio(i,k,iPO4_))
#else
                fac1=dtdays*t_PPmax
...
#if defined TBNT_OUT && defined BIO_SEDIMENT && \
    defined DENITRIFICATION && defined SOLVE3D
                TBNTflx3(i,j,k,iTBNT_NO3_PhyN) =                        &
     &                                 TBNTflx3(i,j,k,iTBNT_NO3_PhyN) + &
# ifdef WET_DRY
     &                                 rmask_full(i,j)*                 &
# endif
     &                                 N_Flux_NewProd * vol(i,k)
                TBNTflx3(i,j,k,iTBNT_NH4_PhyN) =                        &
     &                                 TBNTflx3(i,j,k,iTBNT_NH4_PhyN) + &
# ifdef WET_DRY
     &                                 rmask_full(i,j)*                 &
# endif
     &                                 N_Flux_RegProd * vol(i,k)
#endif
#ifdef OXYGEN
...
#if defined TBNT_OUT && defined BIO_SEDIMENT && \
    defined DENITRIFICATION && defined SOLVE3D
                TBNTflx3(i,j,k,iTBNT_NH4_NO3) =                         &
     &                                TBNTflx3(i,j,k,iTBNT_NH4_NO3) +   &
# ifdef WET_DRY
     &                                rmask_full(i,j)*                  &
# endif
     &                                N_Flux_Nitrifi * vol(i,k)
#endif
#ifdef OXYGEN
                Bio(i,k,iOxyg)=Bio(i,k,iOxyg)-2.0_r8*N_Flux_Nitrifi
# ifdef OXYGEN_VIRT
                Bio(i,k,iOxyV)=Bio(i,k,iOxyV)-2.0_r8*N_Flux_Nitrifi
# endif
#endif
...
#if defined TBNT_OUT && defined BIO_SEDIMENT && \
    defined DENITRIFICATION && defined SOLVE3D
              TBNTflx3(i,j,k,iTBNT_Phy_ZooN) =                          &
     &                   TBNTflx3(i,j,k,iTBNT_Phy_ZooN) +               &
# ifdef WET_DRY
     &                   rmask_full(i,j)*                               &
# endif
     &                   N_Flux_Assim * vol(i,k)
              TBNTflx3(i,j,k,iTBNT_Phy_SDeN) =                          &
     &                   TBNTflx3(i,j,k,iTBNT_Phy_SDeN) +               &
# ifdef WET_DRY
     &                   rmask_full(i,j)*                               &
# endif
     &                   (N_Flux_Egest + N_Flux_Pmortal) * vol(i,k)
#endif
#ifdef CARBON
...
#if defined TBNT_OUT && defined BIO_SEDIMENT && \
    defined DENITRIFICATION && defined SOLVE3D
              TBNTflx3(i,j,k,iTBNT_Zoo_SDeN) =                          &
# ifdef WET_DRY
     &                 rmask_full(i,j)*                                 &
# endif
     &                 TBNTflx3(i,j,k,iTBNT_Zoo_SDen) +                 &
     &                 N_Flux_Zmortal * vol(i,k)

              TBNTflx3(i,j,k,iTBNT_Zoo_NH4) =                           &
     &                 TBNTflx3(i,j,k,iTBNT_Zoo_NH4) +                  &
# ifdef WET_DRY
     &                 rmask_full(i,j)*                                 &
# endif
     &                 (N_Flux_Zmetabo + N_Flux_Zexcret) * vol(i,k)
#endif
#ifdef OXYGEN
              Bio(i,k,iOxyg)=Bio(i,k,iOxyg)-                            &
...
#if defined TBNT_OUT && defined BIO_SEDIMENT && \
    defined DENITRIFICATION && defined SOLVE3D
              TBNTflx3(i,j,k,iTBNT_Phy_LDen) =                          &
     &                                 TBNTflx3(i,j,k,iTBNT_Phy_LDen) + &
# ifdef WET_DRY
     &                                 rmask_full(i,j)*                 &
# endif
     &                                 N_Flux_CoagP * vol(i,k)
              TBNTflx3(i,j,k,iTBNT_SDe_LDen) =                          &
     &                                 TBNTflx3(i,j,k,iTBNT_SDe_LDen) + &
# ifdef WET_DRY
     &                                 rmask_full(i,j)*                 &
# endif
     &                                 N_Flux_CoagD * vol(i,k)
#endif
#ifdef CARBON
...
# ifdef PO4
     ...
     &                       (N_Flux_RemineS+N_Flux_RemineL)
# endif
              Bio(i,k,iOxyg)=Bio(i,k,iOxyg)-rOxNH4 *                    &
     &                       (N_Flux_RemineS+N_Flux_RemineL)
# ifdef OXYGEN_VIRT
              Bio(i,k,iOxyV)=Bio(i,k,iOxyV)-rOxNH4 *                    &
     &                       (N_Flux_RemineS+N_Flux_RemineL)
# endif
# ifdef RIVER_PON
     ...
     &                       N_Flux_RemineR
#  ifdef PO4
              Bio(i,k,iPO4_)=Bio(i,k,iPO4_)+R_P2N(ng)*N_Flux_RemineR
#  endif
              Bio(i,k,iOxyg)=Bio(i,k,iOxyg)-rOxNH4*N_Flux_RemineR
#  ifdef OXYGEN_VIRT
              Bio(i,k,iOxyV)=Bio(i,k,iOxyV)-rOxNH4*N_Flux_RemineR
#  endif
# endif
# if defined TBNT_OUT && defined BIO_SEDIMENT && \
     defined DENITRIFICATION && defined SOLVE3D
              TBNTflx3(i,j,k,iTBNT_SDe_NH4) =                           &
     &                             TBNTflx3(i,j,k,iTBNT_SDe_NH4) +      &
#  ifdef WET_DRY
     &                             rmask_full(i,j)*                     &
#  endif
     &                             N_Flux_RemineS * vol(i,k)
              TBNTflx3(i,j,k,iTBNT_LDe_NH4) =                           &
     &                             TBNTflx3(i,j,k,iTBNT_LDe_NH4) +      &
#  ifdef WET_DRY
     &                             rmask_full(i,j)*                     &
#  endif
     &                             N_Flux_RemineL * vol(i,k)
#  ifdef RIVER_PON
              TBNTflx3(i,j,k,iTBNT_RDe_NH4) =                           &
     &                             TBNTflx3(i,j,k,iTBNT_RDe_NH4) +      &
#   ifdef WET_DRY
     &                             rmask_full(i,j)*                     &
#   endif
     &                             N_Flux_RemineR * vol(i,k)
#  endif
# endif
            END DO
          END DO
...
# endif
# ifdef OXYGEN_VIRT
            O2_virt_Flux=cff3*(O2satu-Bio(i,k,iOxyV))
            Bio(i,k,iOxyV)=Bio(i,k,iOxyV)+                              &
     &                     O2_virt_Flux*Hz_inv(i,k)
#  ifdef DIAGNOSTICS_BIO
            DiaBio2d(i,j,iO2vf)=DiaBio2d(i,j,iO2vf)+                    &
#   ifdef WET_DRY
     &                          rmask_full(i,j)*                        &
#   endif
     &                          O2_virt_Flux*fiter
...
!
            CALL caldate (tdays(ng), yy_i=year, yd_r8=yday)
            pmonth=real(year,r8)-1951.0_r8+yday/365.0_r8
...
#if defined TBNT_OUT && defined BIO_SEDIMENT && defined DENITRIFICATION &&defined SOLVE3D
                IF (((ibio.eq.iPhyt).or.(ibio.eq.iSDeN).or.             &
     &               (ibio.eq.iLDeN)).and.(k>1)) THEN
                  IF (ibio.eq.iPhyt) THEN
                    TBNTsink(i,j,k,iTBNT_snk_PhyN) =                    &
     &                              TBNTsink(i,j,k,iTBNT_snk_PhyN) -    &
# ifdef WET_DRY
     &                              rmask_full(i,j)*                    &
# endif
     &                              FC(i,k-1) * omn(i,j)
                  ELSEIF (ibio.eq.iSDeN) THEN
                    TBNTsink(i,j,k,iTBNT_snk_SDeN) =                    &
     &                              TBNTsink(i,j,k,iTBNT_snk_SDeN) -    &
# ifdef WET_DRY
     &                              rmask_full(i,j)*                    &
# endif
     &                              FC(i,k-1) * omn(i,j)
                  ELSEIF (ibio.eq.iLDeN) THEN
                    TBNTsink(i,j,k,iTBNT_snk_LDeN) =                    &
     &                              TBNTsink(i,j,k,iTBNT_snk_LDeN) -    &
# ifdef WET_DRY
     &                              rmask_full(i,j)*                    &
# endif
...
#  endif
#  if defined TBNT_OUT && defined SOLVE3D
                IF (ibio.eq.iPhyt) THEN
                  TBNTflx2(i,j,iTBNT_sPhy_N2) =                         &
     &                          TBNTflx2(i,j,iTBNT_sPhy_N2) +           &
#   ifdef WET_DRY
     &                          rmask_full(i,j)*                        &
#   endif
     &                          (1.0_r8-cff2)*cff1 * vol(i,1)
                  TBNTflx2(i,j,iTBNT_sPhy_NH4) =                        &
     &                          TBNTflx2(i,j,iTBNT_sPhy_NH4) +          &
#   ifdef WET_DRY
     &                          rmask_full(i,j)*                        &
#   endif
     &                          cff2 * cff1 * vol(i,1)
                ELSEIF (ibio.eq.iSDeN) THEN
                  TBNTflx2(i,j,iTBNT_sSDe_N2) =                         &
     &                          TBNTflx2(i,j,iTBNT_sSDe_N2) +           &
#   ifdef WET_DRY
     &                          rmask_full(i,j)*                        &
#   endif
     &                          (1.0_r8-cff2)*cff1 * vol(i,1)
                  TBNTflx2(i,j,iTBNT_sSDe_NH4) =                        &
     &                          TBNTflx2(i,j,iTBNT_sSDe_NH4) +          &
#   ifdef WET_DRY
     &                          rmask_full(i,j)*                        &
#   endif
     &                          cff2 * cff1 * vol(i,1)
                ELSEIF (ibio.eq.iLDeN) THEN
                  TBNTflx2(i,j,iTBNT_sLDe_N2) =                         &
     &                          TBNTflx2(i,j,iTBNT_sLDe_N2) +           &
#   ifdef WET_DRY
     &                          rmask_full(i,j)*                        &
#   endif
     &                          (1.0_r8-cff2)*cff1 * vol(i,1)
                  TBNTflx2(i,j,iTBNT_sLDe_NH4) =                        &
     &                          TBNTflx2(i,j,iTBNT_sLDe_NH4) +          &
#   ifdef WET_DRY
     &                          rmask_full(i,j)*                        &
#   endif
...
#   ifdef OXYGEN_VIRT
                Bio(i,1,iOxyV)=Bio(i,1,iOxyV)-cff1*cff3
#   endif
#  endif
# else
...
#   ifdef OXYGEN_VIRT
                Bio(i,1,iOxyV)=Bio(i,1,iOxyV)-cff1*cff4
#   endif
#  endif
# endif
              END DO
...
!  temperature is used to impose an oxygen demand (SOD) and return 
!  a proportional source of NH4.
!  (Note: this only works if oxygen is enabled.)
!  (BIO_SEDIMENT should be disabled)
!
          DO i=Istr,Iend
!            cff1=dtdays*0.826747_r8*2.0_r8**(Bio(i,1,itemp)/10.0_r8)
!            cff1=dtdays*1.46608_r8*2.0_r8**(Bio(i,1,itemp)/10.0_r8)
!            cff1=dtdays*6.03392_r8*2.0_r8**(Bio(i,1,itemp)/10.0_r8)
            cff1=dtdays*6.67325_r8*2.0_r8**(Bio(i,1,itemp)/10.0_r8)
            Bio(i,1,iOxyg)=Bio(i,1,iOxyg)-cff1
# ifdef OXYGEN_VIRT
	    Bio(i,1,iOxyV)=Bio(i,1,iOxyV)-cff1
# endif
...
!
!  Particles sink out of the model. A parameterization dependent on  
!  temperature and oxygen is used to impose an oxygen demand (SOD) and return 
!  a proportional source of NH4.
...
!  is used to impose an oxygen demand (SOD) and return 
!  a proportional source of NH4.
!  Parameterization is from Murrell and Lehrter (2010).
!  (Note: this only works if oxygen is enabled.)
!
          DO i=Istr,Iend
            cff=dtdays*(0.094_r8*Bio(i,1,iOxyg)-1.35_r8)
...
!
          DO i=Istr,Iend
            cff=dtdays*0.0235_r8*2.0_r8**(Bio(i,1,itemp)/10.0_r8)*Bio(i,1,iOxyg)
            cff1=MAX(cff,0.0_r8)*Hz_inv(i,1)
...
!  Particles sink out of the model. A parameterization dependent on  
!  temperature is used to impose an oxygen demand (SOD) and return 
!  a proportional source of NH4, PO4 and DIC.
!  Parameterization based on a Q10 formulation fitted to results
!  from the Soetaert et al (1996) diagenetic model for the Louisiana Shelf.
!
          DO i=Istr,Iend
            cff=Bio(i,1,itemp)
#  if defined SED_TEMP_ADJUSTED
            cff1=dtdays*0.7674513574_r8*2.0_r8**(cff/5.9796078232_r8)*  &
     &           Hz_inv(i,1)
#  elif defined SED_TEMP_KDT
            cff1=dtdays*0.6136358592_r8*2.0_r8**(cff/5.4740125158_r8)*  &
     &           Hz_inv(i,1)
#  elif defined SED_TEMP_KDT_FUTURE
            cff1=dtdays*0.5926914788_r8*2.0_r8**(cff/5.9269180602_r8)*  &
     &           Hz_inv(i,1)     
#  else
!            cff=MIN(Bio(i,1,itemp),35.0_r8)
            cff1=dtdays*0.9290945574_r8*2.0_r8**(cff/6.6841529873_r8)*  &
     &           Hz_inv(i,1)
#  endif
