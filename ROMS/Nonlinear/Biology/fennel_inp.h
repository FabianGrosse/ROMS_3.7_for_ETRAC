...
!
!svn $Id: fennel_inp.h 830 2017-01-24 21:21:11Z arango $
!================================================== Hernan G. Arango ===
...
!
    ...
    defined SOLVE3D
      integer  :: itbnt
      logical, dimension(Ngrids) :: Ltbnt
# endif
...
# endif
            ...
            CASE ('Dout(iNifx)')
              ...
              DO ng=1,Ngrids
                Dout(i,ng)=Lbio(ng)
              END DO
# if defined OXYGEN && defined OXYGEN_VIRT 
            CASE ('Dout(iO2vf)')
              IF (iDbio2(iO2vf).eq.0) THEN
                IF (Master) WRITE (out,40) 'iDbio2(iO2vf)'
                exit_flag=5
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Lbio)
...
# endif
    ...
    defined DENITRIFICATION && defined SOLVE3D
            CASE ('TBNTout(NITROGEN)')
              Npts=load_l(Nval, Cval, Ngrids, Ltbnt)
              ! volume
              IF (idTBNTgvol.eq.0) THEN
                IF (Master) WRITE (out,40) 'idTBNTgvol'
                exit_flag=5
                RETURN
              END IF
              DO ng = 1,Ngrids
                TBNTout(idTBNTgvol,ng) = Ltbnt(ng)
              END DO
              ! 3D tracers
              DO itbnt = 1,nTBNTtrc3
                i=idTBNTtrc3(itbnt)
                IF (i.eq.0) THEN
                  IF (Master) WRITE (out,40) 'idTBNTtrc3(', itbnt, ')'
                  exit_flag=5
                  RETURN
                END IF
                DO ng = 1,Ngrids
                  TBNTout(i,ng) = Ltbnt(ng)
                END DO
              END DO
              DO itbnt = 1,nTBNTflx2
                i=idTBNTflx2(itbnt)
                IF (i.eq.0) THEN
                  IF (Master) WRITE (out,40) 'idTBNTflx2(', itbnt, ')'
                  exit_flag=5
                  RETURN
                END IF
                DO ng = 1,Ngrids
              ...
              DO itbnt = 1,nTBNTflx3
                i=idTBNTflx3(itbnt)
                IF (i.eq.0) THEN
                  IF (Master) WRITE (out,40) 'idTBNTflx3(', itbnt, ')'
                  exit_flag=5
                  RETURN
                END IF
                DO ng = 1,Ngrids
              ...
              DO itbnt = 1,nTBNTsink
                i=idTBNTsink(itbnt)
                IF (i.eq.0) THEN
                  IF (Master) WRITE (out,40) 'idTBNTsink(', itbnt, ')'
                  exit_flag=5
                  RETURN
                END IF
                DO ng = 1,Ngrids
              ...
              DO ng = 1,Ngrids
                DO itrc=1,NTBNTTRC(ng)
                  DO itbnt=1,nTBNTphys
                    i=idTBNTphys(itrc,itbnt)
                    IF (i.eq.0) THEN
                      IF (Master) WRITE (out,40) 'idTBNTphys(',           &
     &                                           itrc, ',', itbnt, ')'
                      exit_flag=5
                      RETURN
                    END IF
                  ...
                  DO itbnt = 1,nTBNTpsrc
                    i=idTBNTpsrc(itrc,itbnt)
                    IF (i.eq.0) THEN
                      IF (Master) WRITE (out,40) 'idTBNTpsrc(',itrc,',',  &
     &                                           itrc, ',', itbnt, ')'
                      exit_flag=5
                      RETURN
                    END IF
...
#if defined TBNT_OUT && defined BIO_SEDIMENT && \
    defined DENITRIFICATION && defined SOLVE3D
            IF (nTBNT(ng).gt.0) THEN
              ! volume
              IF (TBNTout(idTBNTgvol,ng)) write(out,130)                  &
     &          TBNTout(idTBNTgvol,ng), 'TBNTout(iTBNTgvol)',             &
     &          'Write TBNT for', TRIM(Vname(1,idTBNTgvol))
              ! 3D tracers
              DO itbnt = 1,nTBNTtrc3
                i=idTBNTtrc3(itbnt)
                IF (TBNTout(i,ng)) WRITE (out,130)                        &
     &            TBNTout(i,ng), 'TBNTout(iTBNTtrc3)',                    &
     &            'Write TBNT for', TRIM(Vname(1,i))
              END DO
              DO itbnt = 1,nTBNTflx2
                i=idTBNTflx2(itbnt)
                IF (TBNTout(i,ng)) WRITE (out,130)                        &
     &            TBNTout(i,ng), 'TBNTout(iTBNTflx2)',                    &
     &            'Write TBNT for', TRIM(Vname(1,i))
              END DO
              DO itbnt = 1,nTBNTflx3
                i=idTBNTflx3(itbnt)
                IF (TBNTout(i,ng)) WRITE (out,130)                        &
     &            TBNTout(i,ng), 'TBNTout(iTBNTflx3)',                    &
     &            'Write TBNT for', TRIM(Vname(1,i))
              END DO
              DO itbnt = 1,nTBNTsink
                i=idTBNTsink(itbnt)
                IF (TBNTout(i,ng)) WRITE (out,130)                        &
     &            TBNTout(i,ng), 'TBNTout(iTBNTsink)',                    &
     &            'Write TBNT for', TRIM(Vname(1,i))
              END DO
              DO itrc=1,NTBNTTRC(ng)
                DO itbnt=1,nTBNTphys
                  i=idTBNTphys(itrc,itbnt)
                  IF (TBNTout(i,ng)) WRITE (out,130)                      &
     &              TBNTout(i,ng), 'TBNTout(iTBNTphys)',                  &
     &              'Write TBNT for', TRIM(Vname(1,i))
                END DO
                DO itbnt = 1,nTBNTpsrc
                  i=idTBNTpsrc(itrc,itbnt)
                  IF (TBNTout(i,ng)) WRITE (out,130)                      &
     &              TBNTout(i,ng), 'TBNTout(iTBNTpsrc)',                  &
     &              'Write TBNT for', TRIM(Vname(1,i))
                END DO
              END DO
            END IF
