...
!
!svn $Id: oxysink_inp.h 810 2016-08-05 01:58:16Z arango $
!  Modified from fennel_inp.h
!================================================== Hernan G. Arango ===
...
!=======================================================================
!                                                                      !
!  This routine reads in the simple oxygen sink model input            !
!  parameters. They are specified in input script "oxysink.in".        !
!                                                                      !
!=======================================================================
!
      USE mod_param
...
!
!-----------------------------------------------------------------------
!  Read in simple oxygen sink model parameters.
!-----------------------------------------------------------------------
!
      DO WHILE (.TRUE.)
        ...
        IF (status.gt.0) THEN
          SELECT CASE (TRIM(KeyWord))
            ...
            CASE ('OxyCCR')
              Npts=load_r(Nval, Rval, Ngrids, OxyCCR)
            CASE ('OxyCCR_hmin')
              Npts=load_r(Nval, Rval, Ngrids, OxyCCR_hmin)
            CASE ('OxyCCR_hmax')
              Npts=load_r(Nval, Rval, Ngrids, OxyCCR_hmax)
            CASE ('TNU2')
...
!
     ...
     &            'Number of iterations for nonlinear convergence.'
            WRITE (out,80) OxyCCR(ng), 'OxyCCR',                        &
     &            'Constant respiration rate (mmol/(m3 day)).'
            WRITE (out,80) OxyCCR_hmin(ng), 'OxyCCR_hmin',              &
     &            'Depth at which OxyCCR starts to decrease (m).'
            WRITE (out,80) OxyCCR_hmax(ng), 'OxyCCR_hmax',              &
     &            'Depth at which OxyCCR goes to zero (m).'
#ifdef TS_DIF2
            DO itrc=1,NBT
              i=idbio(itrc)
...
!
  ...
  60  FORMAT (/,/,' Simple O2 Model Parameters, Grid: ',i2.2,           &
     &        /,  ' =================================',/)
