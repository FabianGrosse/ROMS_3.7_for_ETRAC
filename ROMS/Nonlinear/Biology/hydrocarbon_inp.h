...
!
!svn $Id: fennel_inp.h 810 2016-08-05 01:58:16Z arango $
!================================================== Hernan G. Arango ===
...
!=======================================================================
!                                                                      !
!  This routine reads in the hydrocarbon model input                   !
!  parameters. They are specified in input script "hydrocarbon.in".    !
!                                                                      !
!=======================================================================
!
      USE mod_param
...
!
!-----------------------------------------------------------------------
!  Read in hydrocarbon model parameters.
!-----------------------------------------------------------------------
!
      DO WHILE (.TRUE.)
        ...
        IF (status.gt.0) THEN
          SELECT CASE (TRIM(KeyWord))
            ...
            CASE ('HydroCDR')
              Npts=load_r(Nval, Rval, Ngrids, HydroCDR)
            CASE ('Oxy2HC')
              Npts=load_r(Nval, Rval, Ngrids, Oxy2HC)
            CASE ('nday_startoil')
              Npts=load_r(Nval, Rval, Ngrids, nday_startoil)              
            CASE ('nday_endoil')
              Npts=load_r(Nval, Rval, Ngrids, nday_endoil)
            CASE ('iloc_oil')
              Npts=load_r(Nval, Rval, Ngrids, iloc_oil)
            CASE ('jloc_oil')
              Npts=load_r(Nval, Rval, Ngrids, jloc_oil)  
            CASE ('kloc_oil')
              Npts=load_r(Nval, Rval, Ngrids, kloc_oil)  
            CASE ('density_oil')
              Npts=load_r(Nval, Rval, Ngrids, density_oil) 
            CASE ('TNU2')
...
!
     ...
     &            'Number of iterations for nonlinear convergence.'
            WRITE (out,80) HydroCDR(ng), 'HydroCDR',                    &
     &            'Hydrocarbon decay rate (day-1).'
            WRITE (out,80) Oxy2HC(ng), 'Oxy2HC',                        &
     &            'ratio of oxygen consumption to hydroC decay (mmol_O2/(mmol_hydroC)).'
            WRITE (out,80) nday_startoil(ng), 'nday_startoil',                        &
     &            'starting day of oil spill.'
            WRITE (out,80) nday_endoil(ng), 'nday_endoil',                            &
     &            'ending day of oil spill.'
            WRITE (out,80) iloc_oil(ng), 'iloc_oil',                                  &
     &            'longitude (i) index of the model grid where oil spill is located.'
            WRITE (out,80) jloc_oil(ng), 'jloc_oil',                                  &
     &            'latitude (j) index of the model grid where oil spill is located.'
            WRITE (out,80) kloc_oil(ng), 'kloc_oil',                                  &
     &            'vertical (k) index of the model grid where oil spill is located.'
            WRITE (out,80) density_oil(ng), 'density_oil',                            &
     &            'density of the oil added to the grid cell at each time step.'
#ifdef TS_DIF2
            DO itrc=1,NBT
              i=idbio(itrc)
...
!
  ...
  60  FORMAT (/,/,' Hydrocarbon Model Parameters, Grid: ',i2.2,         &
     &        /,  ' =================================',/)
