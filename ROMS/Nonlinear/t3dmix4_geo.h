...
!
!svn $Id: t3dmix4_geo.h 854 2017-07-18 23:28:45Z arango $
!***********************************************************************
...
!
#ifdef PROFILE
      CALL wclock_on (ng, iNLM, 28, __LINE__, __FILE__)
#endif
...
# endif
    ...
    defined SOLVE3D
     ...
     &                   OCEAN(ng) % t)
#ifdef PROFILE
      CALL wclock_off (ng, iNLM, 28, __LINE__, __FILE__)
#endif
...
!
    ...
    defined SOLVE3D
                IF (itbnt>0) THEN
                  TBNTphysWRK(i,j,k,itbnt,iTBNT_hDix) = FX(i,j)*dt(ng)
                  TBNTphysWRK(i,j,k,itbnt,iTBNT_hDie) = FE(i,j)*dt(ng)
                  TBNTphysWRK(i,j,k,itbnt,iTBNT_hDis) = FS(i,j,k1)*     &
     &                                                  dt(ng)*omn(i,j)
                END IF
#endif
              END DO
